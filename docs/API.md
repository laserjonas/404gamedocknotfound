...
| PATCH  | `/instances/:id`       | operator | Body `{name?, autoStart?, crashRestart?, backupIntervalHours?, backupRetentionCount?, restartIntervalHours?, memoryMaxMb?, cpuQuotaPercent?, startExecutable?, startArgs?, envVars?, variables?, ports?}` - port variables and `ports` entries are kept in sync with each other; a port already claimed by another instance gives `409`; non-null resource limits give `400` unless the instance is isolated |
